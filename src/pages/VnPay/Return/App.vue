...
<template>
  <div class="card">
    ...
    <div v-if="success" class="text-900 font-bold text-4xl my-2">Successful Order 🚀</div>
    <div v-else class="text-900 font-bold text-4xl my-2">Failed Order 😢</div>
    <p v-if="success" class="text-700 text-xl mt-0 mb-4 p-0">
      Your order is on the way. It'll be shipped today. We'll inform you. PaymentId: {{ paymentId }}
    </p>
    <p v-else class="text-700 text-xl mt-0 mb-4 p-0">
      Your order is failed. Please try again. Reason: {{ reason }}
    </p>
    <div
      style="
        height: 3px;
    ...
    >
      ...
      <div>
        <button
          ...
          label="Details"
          icon="pi pi-list"
          class="p-element p-ripple p-button-outlined p-button-primary mr-2 p-button p-component"
        >
          <span
            class="p-button-icon p-button-icon-left pi pi-list"
            aria-hidden="true"
          ></span
        ...
        >
          <span
            class="p-button-icon p-button-icon-left pi pi-print"
            aria-hidden="true"
          ></span
